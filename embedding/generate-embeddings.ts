...
const ignoredFiles = ["markdowns/404.mdx"];
...
class MarkdownEmbeddingSource extends BaseEmbeddingSource {
  ...
  constructor(
    ...
    const path = filePath.replace(/^markdowns/, "").replace(/\.mdx?$/, "");
...
async function generateEmbeddings() {
  ...
  const embeddingSources: EmbeddingSource[] = [
    ...(await walk("markdowns"))
      .filter(({ path }) => /\.mdx?$/.test(path))
      .filter(({ path }) => !ignoredFiles.includes(path))
      .map((entry) => new MarkdownEmbeddingSource("guide", entry.path)),
  ];

  console.log(`Discovered ${embeddingSources.length} documents`);
  // console.log(embeddingSources)

  if (!shouldRefresh) {
    console.log("Checking which documents are new or have changed");
  } else {
    console.log("Refresh flag set, re-generating all docs");
  }
  ...
  for (const embeddingSource of embeddingSources) {
    ...
    try {
      ...
      if (
        !shouldRefresh &&
        existingPage !== null &&
        existingPage.checksum === checksum
      ) {
        const existingParentPage =
          existingPage.parentPage?.length === 1
